...
import { generateDraftWithGrok } from '../services/grokAI';
...
const getFormFieldConfig = (template: any, field: string): FormFieldConfig | undefined => {
  const formFields = template.uiHints?.formFields as Record<string, FormFieldConfig> | undefined;
  ...
  const handleGenerateDraft = async () => {
    ...
    try {
      ...
      template.template.forEach((section) => {
        if (section.heading) {
          templateContent += section.heading + '\n\n';
        }
        if (section.content) {
      ...
      // Call Grok AI to generate the draft
      ...
      // Create draft object with AI-generated content
      const now = new Date();
      const draft: SavedDraft = {
        draftId: `draft-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        title: template.title,
        body: response.generatedContent,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-border">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {/* Error Message */}
          {error && (
            <div className="mb-4 p-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg flex items-start gap-3">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      {template.requiredInputs.map((field) => {
                        const fieldConfig = getFormFieldConfig(template, field);
                        const fieldType = fieldConfig?.type || 'text';
                        const placeholder = fieldConfig?.placeholder || field;
                        const fieldOptions: string[] = fieldConfig?.options ?? [];
          ...
          <div className="px-6 py-4 border-t border-border">
            ...
            {generating && (
              <div className="mb-3 p-3 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-medium text-blue-900 dark:text-blue-200">Generating Draft with Grok AI</h4>
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                      Please wait while we create your legal document...
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-3">
              <button
                onClick={onClose}
                disabled={generating}
                className="px-6 py-2 border border-border text-foreground rounded-lg hover:bg-accent transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancel
              </button>
              <button
