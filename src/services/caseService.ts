...
export function createCase(input: CreateCaseInput): CaseData {
  ...
  const newCase: CaseData = {
    ...
    // Drafts - use generated drafts with content if available
    drafts: input.generatedDrafts && input.generatedDrafts.length > 0
      ? input.generatedDrafts.map((draft: { type: string; title: string; content: string }, idx: number) => ({
      ...
      : input.selectedDrafts.map((draftType, idx) => ({
          id: `draft-${idx + 1}`,
